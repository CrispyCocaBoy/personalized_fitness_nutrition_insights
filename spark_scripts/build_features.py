...
from pyspark.sql.types import StructType, StructField, StringType, IntegerType, DoubleType, TimestampType # Per definire lo schema

def get_spark_session():
    """
    Configura e restituisce una sessione Spark.
    La configurazione per MinIO viene passata da Airflow, rendendo lo script più portabile.
    """
...
def process_data(spark: SparkSession, bronze_base_path: str, gold_base_path: str):
    ...
    # Nutrition
    feedback_nutrition_df = spark.read.option("header", "true").option("inferSchema", "true").csv(f"{bronze_base_path}/user_feedback_nutrition.csv")
    ...
    # Workout
    feedback_workout_df = spark.read.option("header", "true").option("inferSchema", "true").csv(f"{bronze_base_path}/user_feedback_workout.csv")
    ...
    metrics_df = spark.read.option("header", "true").option("inferSchema", "true").csv(f"{bronze_base_path}/daily_user_metrics_live.csv") \
